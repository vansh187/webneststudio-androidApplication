...
type Props = {
  children: ReactNode;
  /** 'screen' = full-bleed fallback; 'inline' = compact card for a list row/bubble. */
...
/**
 * Belt-and-braces boundary for the chat feature. Every chat screen is wrapped in
 * a `screen` boundary; the message list and each bubble are wrapped in `inline`
 * boundaries so one malformed payload can never blank the whole conversation.
 */
export class ChatErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error(`[chat] ${this.props.label ?? 'boundary'} caught`, error, info.componentStack);
  }
